...
CIsys_LevitateAttacked::CIsys_LevitateAttacked()
	: CState(BOSS_LEVITATE_ATTACKED)
	, m_dwTime(0)
{
}
...
void CIsys_LevitateAttacked::Update()
{
	// Player의 위치 체크
...
void CIsys_LevitateAttacked::LateUpdate()
{
}

void CIsys_LevitateAttacked::Enter()
{
	CMonster* pMonster = GetMonster();
	CObj* pPlayer = CObjMgr::Get_Instance()->GetObjList(OBJ_PLAYER).front();
...
void CIsys_LevitateAttacked::Exit()
{
	CMonster* pMonster = GetMonster();
	pMonster->Set_dwTime(0);
}
