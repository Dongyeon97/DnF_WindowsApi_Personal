...
CIsys_ThrustAttacked::CIsys_ThrustAttacked()
	: CState(BOSS_THRUST_ATTACKED)
	, m_dwTime(0)
{
}
...
void CIsys_ThrustAttacked::Update()
{
	// Player의 위치 체크
...
void CIsys_ThrustAttacked::LateUpdate()
{
}

void CIsys_ThrustAttacked::Enter()
{
	CMonster* pMonster = GetMonster();
	CObj* pPlayer = CObjMgr::Get_Instance()->GetObjList(OBJ_PLAYER).front();
...
void CIsys_ThrustAttacked::Exit()
{
	CMonster* pMonster = GetMonster();
	pMonster->Set_dwTime(0);
}
