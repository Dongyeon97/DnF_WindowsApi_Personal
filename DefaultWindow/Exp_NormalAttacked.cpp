...
CExp_NormalAttacked::CExp_NormalAttacked()
	: CState(MON_NORMAL_ATTACKED)
	, m_dwTime(0)
{
}
...
void CExp_NormalAttacked::Update()
{
	// Player의 위치 체크
...
void CExp_NormalAttacked::LateUpdate()
{
}

void CExp_NormalAttacked::Enter()
{
	CMonster* pMonster = GetMonster();
	CObj* pPlayer = CObjMgr::Get_Instance()->GetObjList(OBJ_PLAYER).front();
