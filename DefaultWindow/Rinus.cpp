...
void CRinus::Initialize()
{
	// 스테이터스
	m_tInfo = { (ALFHLYRA_SIZEX * 0.75f), 350.f , 80.f, 140.f };
	...
	// 상하좌우 방향 상태 이미지 불러오기
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/NPC/라이너스/Rainus.bmp", L"Rinus");
}
...
void CRinus::Render(HDC hDC)
{
	...
	if (0 < m_TextTerm && 6 > m_TextTerm)
	{
		...
		TextOut(hDC, m_tInfo.fX + fScrollX - 67, m_tInfo.fY + fScrollY - 90, str1.c_str(), str1.size());
