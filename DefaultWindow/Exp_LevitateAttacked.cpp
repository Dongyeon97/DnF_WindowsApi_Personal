...
CExp_LevitateAttacked::CExp_LevitateAttacked()
	: CState(MON_LEVITATE_ATTACKED)
	, m_dwTime(0)
{
}
...
void CExp_LevitateAttacked::Update()
{
	// Player의 위치 체크
...
void CExp_LevitateAttacked::LateUpdate()
{
}

void CExp_LevitateAttacked::Enter()
{
	CMonster* pMonster = GetMonster();
	CObj* pPlayer = CObjMgr::Get_Instance()->GetObjList(OBJ_PLAYER).front();
...
void CExp_LevitateAttacked::Exit()
{
	CMonster* pMonster = GetMonster();
	pMonster->Set_dwTime(0);
}
