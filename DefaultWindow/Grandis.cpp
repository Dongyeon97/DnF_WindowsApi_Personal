...
void CGrandis::Initialize()
{
	// 스테이터스
	m_tInfo = { 1000.f, 350.f , 97.f, 121.f };
	...
	// 피격 Rect 크기
	m_fCollisionSizeX = 97.f;
	m_fCollisionSizeY = 121.f;
	...
	m_pFrameKey = L"Grandis";

	// 상하좌우 방향 상태 이미지 불러오기
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/NPC/그란디스/Grandis.bmp", L"Grandis");
}
...
int CGrandis::Update()
{
	...
	if (6 < m_dwTime && 400.f > fDistance)
	{
		...
		{
		case 0:
			CSoundMgr::Get_Instance()->PlaySoundW(L"gr_amb_01.ogg", SOUND_EFFECT, g_fVolume);
			break;

		case 1:
			CSoundMgr::Get_Instance()->PlaySoundW(L"gr_amb_02.ogg", SOUND_EFFECT, g_fVolume);
			break;

		case 2:
			CSoundMgr::Get_Instance()->PlaySoundW(L"gr_amb_03.ogg", SOUND_EFFECT, g_fVolume);
			break;
		}
	}
...
void CGrandis::Render(HDC hDC)
{
	...
	{
		wstring str1 = L"곰인형은 신경쓰지 말아주실래요?";
...
void CGrandis::Release()
{

}

void CGrandis::Update_Rect()
{
	// 오브젝트 실제 크기 업데이트
